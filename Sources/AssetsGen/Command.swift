...
class Command {
	...
	let resourcesPathArg: OptionArgument<String>
	...
	init() throws {
		...
		resourcesPathArg = parser.add(
			option: "--resources-path",
			kind: String.self,
			usage: "eg. --resources-path ./input/resources"
		)
	...
	func run() throws {
		...
		let resourcesPath = args.get(resourcesPathArg)
		...
		case "gen-img":
			guard let resourcesPath = resourcesPath else {
				throw CommandError.missingResourcesPath
			}
	...
	func generateImages(
		...
		let generator = AssetsGenerator(inputPath: inputPath)
		generator.generate(
			outputPath: outputPath,
			resourcesPath: resourcesPath,
